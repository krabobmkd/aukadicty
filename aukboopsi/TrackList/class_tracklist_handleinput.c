...
// ULONG TrackList_DoNotify(struct IClass *C, struct Gadget *Gad, Msg M, ULONG Flags, Tag Tags, ...);
ULONG TrackList_NotifyCoords(Class *C, struct Gadget *Gad, struct GadgetInfo	*GInfo)
{
    struct opUpdate notifymsg;
    TrackList *gdata=INST_DATA(C, Gad);
    ULONG tags[]={
        GA_ID,0,
        TRACKLIST_CenterX,0,
        TRACKLIST_CenterY,0,
        ...
        case 0x4c: // UP
          {
            ...
            TrackList_Notify(C,Gad,(APTR)Input, 0);
          }
          break;
...
//          DKP("  c=%ld r=%ld\n", c, r);
          ...
          if(c<gdata->Cols && r<gdata->Rows)
          {
            ...
            if(gdata->ActivePen!=gdata->LastActivePen)
            {
              ...
              TrackList_Notify(C,Gad,(APTR)Input, OPUF_INTERIM);
              gad_Render(C,Gad,(APTR)Input,GREDRAW_UPDATE);
            }
          }
...
//              retval = GMR_NOREUSE;
          ...
          case SELECTDOWN:
            ...
            {// outside gadget or disabled.
              ...
              {
                gdata->_EditMode=0;
                TrackList_Render(C,Gad,(APTR)Input,GREDRAW_UPDATE);
              }
//              retval = GMR_NOREUSE | GMR_VERIFY;
            ...
            if(Gad->Height>0)
              ...
              SetGadgetAttrs(Gad,Input->gpi_GInfo->gi_Window,NULL,
                    TRACKLIST_CenterX,cx,
                    TRACKLIST_CenterY,cy,
                    TAG_END
                );
              //TrackList_Render(C,Gad,(APTR)Input,GREDRAW_UPDATE);
       ...
       /*   case MENUDOWN:
          if(gdata->EditMode)//                                                                      (44.3.1) (09/01/00)
            {//                                                                                        (44.3.1) (09/01/00)
              ...
              TrackList_Notify(C,Gad,(APTR)Input, 0);//                                                        (44.3.1) (09/01/00)
            }//                                                                                        (44.3.1) (09/01/00)
            retval = GMR_REUSE;*/
...
/*
    ...
    // if(notifCoords)
    // {
    //     TrackList_NotifyCoords(C,Gad,Input->gpi_GInfo);
    // }
