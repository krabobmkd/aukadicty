...
struct Library *TimeRuleBase=NULL;
#endif
    ...
    case OM_UPDATE:
        {
            ...
            if( sender_ID == GAD_TIMERULE_TOTEST )
            {   // table used as parameter for the button internal sprintf() formating
                LONG centerXY[2];
                if((ptag = FindTagItem( TIMERULE_CenterX,M->opUpdate.opu_AttrList ))!=NULL)
                    centerXY[0] = ((UWORD)ptag->ti_Data * 100)>>16; // get percent
                if((ptag = FindTagItem( TIMERULE_CenterY,M->opUpdate.opu_AttrList ))!=NULL)
...
//        cleanexit("Can't open bitmap.image");
   ...
   if ( ! (CheckBoxBase = OpenLibrary("gadgets/checkbox.gadget",44)))
    ...
    if(TimeRuleStaticInit()) cleanexit("Can't create private class");
#else
    if ( ! (TimeRuleBase = OpenLibrary("timerule.gadget",VERSION_TIMERULE)))
        cleanexit("Can't open timerule.gadget");
#endif
    ...
    // with TIMERULE_STATICLINK, TIMERULE_GetClass() is a function,
    //  else it is a vector function from a shared class library.
    app->kbdview = NewObject(TIMERULE_GetClass(), NULL,
        GA_DrawInfo, app->drawInfo,
        GA_ID,      GAD_TIMERULE_TOTEST, // Gadget ID assigned by the application, needed to sort notifies.
                        ...
                        switch (result & WMHI_GADGETMASK)
                        {
                            case GAD_BUTTON_RECENTER:
                                ...
                                SetGadgetAttrs((struct Gadget *)app->kbdview,app->win,NULL,
                                    TIMERULE_CenterX, 32768,
                                    TIMERULE_CenterY, 32768,
                                    TAG_DONE);
                            break;
...
#ifndef TIMERULE_STATICLINK
    if(TimeRuleBase) CloseLibrary(TimeRuleBase);
#else
    TimeRuleStaticClose();
#endif
